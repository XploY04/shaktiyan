#!/usr/bin/env bash
# Installs every skill in this repo for Claude Code, ChatGPT, and Codex.
# Usage: ./install.sh [--link] [--rules] [--no-claude]
set -euo pipefail

repo=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
claude_dest="${CLAUDE_HOME:-$HOME/.claude}/skills"
openai_dest="${OPENAI_SKILLS_HOME:-$HOME/.agents/skills}"
...
install_skills() {
  ...
  for skill in "$repo"/skills/*/; do
    ...
    if [ -f "$target/scripts/package.json" ]; then
      if command -v npm >/dev/null 2>&1; then
        echo "installing node deps for $name in $agent"
  ...
  echo "installed ${#installed[@]} skill(s) for $agent into $dest: ${installed[*]}"
}

install_skills "$claude_dest" "Claude Code"
install_skills "$openai_dest" "ChatGPT and Codex"
...
echo "restart Claude Code, ChatGPT, and Codex to pick the skills up"
